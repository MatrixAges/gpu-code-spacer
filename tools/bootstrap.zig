...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len == 2 and (std.mem.eql(u8, args[1], "--ggml") or windows)) {
        ...
        const cmake_args = [_][]const u8{
            ...
            "-DGGML_BUILD_EXAMPLES=OFF",  "-DGGML_OPENMP=OFF",       "-DGGML_BLAS=OFF",       "-DGGML_NATIVE=OFF",             install_arg,
        ...
        _ = try command(allocator, init.io, &.{ "cmake", "--build", build_dir, "--config", "Release", "--parallel", "4" });
        _ = try command(allocator, init.io, &.{ "cmake", "--install", build_dir, "--config", "Release" });
